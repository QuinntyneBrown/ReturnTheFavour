...
try
{
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "ReturnTheFavour");
    ...
    {
        var context = scope.ServiceProvider.GetRequiredService<ReturnTheFavourDbContext>();
        ...
        {
            context.Database.ExecuteSql($"DROP SCHEMA [ReturnTheFavour]");

            context.Database.ExecuteSql($"DELETE from __EFMigrationsHistory where MigrationId like '%_ReturnTheFavour_%';");
        }
